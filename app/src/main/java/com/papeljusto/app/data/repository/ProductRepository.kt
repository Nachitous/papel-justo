...
class ProductRepository(private val context: Context)
{
    ...
    fun getAllProducts(): Flow<List<Product>> = context.dataStore.data.map { prefs ->
        ...
        runCatching { json.decodeFromString<List<Product>>(raw) }.getOrDefault(emptyList())
    }
    ...
    private fun currentList(prefs: Preferences): List<Product>
    {
        ...
        return runCatching { json.decodeFromString<List<Product>>(raw) }.getOrDefault(emptyList())
    }
}
